...
$(window).on("load resize", debounce(function () {
    ...
    if (window.innerWidth <= 900) {	// ブレイクポイントを900pxに修正
        console.log("小さな端末用の処理を実行");
        // 小さな端末用の処理
        $('body').addClass('small-screen').removeClass('large-screen');
        $menubar.addClass('display-none').removeClass('display-block');
        $menubarHdr.removeClass('display-none').addClass('display-block');
        console.log("ハンバーガーメニュー表示状態: ", $menubarHdr.attr('class'));
    } else {
        console.log("大きな端末用の処理を実行");
        // 大きな端末用の処理
...
$(function () {
    ...
    if (window.innerWidth <= 900) {
        $menubarHdr.addClass('display-block').removeClass('display-none');
    }
